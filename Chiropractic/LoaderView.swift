...
//  Chiropractic
//
//  Created by Nicholas Laughter on 2/27/17.
...
class LoaderView: UIView {
    ...
    class var sharedInstance: LoaderView {
        struct Singleton {
            static let instance = LoaderView(frame: CGRect(origin: CGPoint(x: 0,y: 0),size: CGSize(width: Config().size,height: Config().size)))
        }
